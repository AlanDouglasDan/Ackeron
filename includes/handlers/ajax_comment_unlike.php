...
if(mysqli_num_rows($check) == 1){
    ...
    $insert_user = mysqli_query($con, "DELETE FROM comment_likes WHERE username='$userLoggedIn' AND comment_id='$id'");    
    if($total_likes == 1)
        $total_likes .= " Like";
    else
        $total_likes .= " Likes";
...
echo '<label onclick="c_like('.$id.')"><span onclick="sound.play()" class="fa fa-heart-o fa-lg"></span> '.$total_likes.'</label>';
?>